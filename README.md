...
Прежде чем что либо обновлять, выполните резервное копирование файлов, как обычно, и затем **обязательно** выполните резервное копирование базы данных в SQL-скрипт:
```
source alias.bash

backup-make

sudo sh -c "docker compose -p ${PROJECT_NAME} exec postgres pg_dump -U ${POSTGRES_USER} --no-acl -Fp -Z 5 ${POSTGRES_DB} > ~/backup_db_before_bundle_update.sql.gz"
```
...
Если вы производили тонкие настройки PostgreSQL, скопируйте файл data/db/postgresql.conf в надежное место, чтобы позже использовать параметры из него для настройки нового файла postgresql.conf новой версии:
```
sudo cp data/db/postgresql.conf ~/
```

Теперь нужно удалить старый контейнер cо старой версией PostgreSQL и со всеми данными (volume) и конфигурацями этого контейнера. Еще раз убедитесь, что у вас есть резервная копия вашей базы данных.
Если вы уверенны, что резервная копия есть, выполните команду:
```
${DOCKER_COMPOSE} rm postgres -s -v
```

До выпуска стабильного релиза userside 3.18, текущая ветка бандла v3.18 также имеет стутс *Beta* и, поэтому, не влита в ветку master. Поэтому, перед обновлением, вам необходимо переключиться на ветку v3.18:
```
sudo git checkout v3.18
sudo git pull
```

Новая версия 3.18 бандла имеет значительные отличия по настройкам:
...
+ переменая `TZ` более не используется и может быть удалена.
...
##### файл compose.yaml
В связи с изменениями версий используемых Docker-образов, в файле произошли некоторые изменения:
+ сервис `postgres` теперь использует Docker-образ, основанный на **postgresql 15** (все это время использовалась 11 версия). Поэтому, перед обновлением, обязательно необходимо сделать резервную копию базы данных в SQL-скрипт, чтобы иметь возможность восстановления на 15й версии. Также у сервиса можно удалить volume `backup:/backup` — в нем больше нет необходимости, т.к. работа с резервными копиями теперь выполняется на хосте, используя сокращенные команды из `bundle.bash`.
...
Если вы производили какие-то тонкие настройки в файле data/db/postgresql.conf, резервную копию которого вы сделали до начала обновления, то восстановите эти настройки в новом файле, но только ни в коем случае не копируйте старый файл от 11й версии поверх нового файла. Очень очень сильно отличаются.
...
```
gunzip < ~/backup_db_before_bundle_update.sql.gz | pgsql
```
...
```
psql -c "SELECT value FROM key_value WHERE key='installer.release'"
```
Запрос должен вернуть текущую версию Userside.
...
+ `pgsql` — запуск консольной утилиты **psql** для взаимодействия с базой данных
...
```
gunzip < data/backup/userside_db_2023-04-07_16-25.sql.gz | pgsql
```
